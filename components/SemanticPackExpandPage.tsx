...
import { Wand2, ArrowLeft, Copy, Download, Save, Loader2 } from 'lucide-react';
...
interface SemanticPackExpandPageProps {
  theme: Theme;
  taskId: string;
  onBack: () => void;
}

const SemanticPackExpandPage: React.FC<SemanticPackExpandPageProps> = ({ theme, taskId, onBack }) => {
  const isDark = theme === 'dark';

  const [semanticKeywords, setSemanticKeywords] = useState<Array<{ text: string; score: number }>>([]);
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        const res = await knowledgeAPI.getExtractionTaskKeywords(taskId);
        const kws = (res.semantic_keywords || res.keywords || []).map((k: any) =>
          typeof k === 'string' ? { text: k, score: 80 } : { text: k?.text ?? '', score: k?.score ?? 80 }
        ).filter((k: { text: string }) => k.text.trim());
        setSemanticKeywords(kws);
      } catch {
        setSemanticKeywords([]);
      } finally {
  ...
  const insertKeyword = (kw: string) => {
    ...
    if (!trimmed) return;
    const lines = parseWords(coreWords);
    if (lines.includes(trimmed)) return;
    setCoreWords(lines.length ? [...lines, trimmed].join('\n') : trimmed);
  };
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    a.download = `词条扩展_${new Date().toISOString().slice(0, 10)}.txt`;
    a.click();
    ...
    <div className={`h-full flex flex-col ${isDark ? 'bg-geo-bg' : 'bg-white'}`}>
      <div className={`flex-shrink-0 flex items-center justify-between px-6 py-4 border-b ${isDark ? 'border-geo-border' : 'border-slate-200'}`}>
        <div className="flex items-center gap-4">
          ...
          <h1 className={`text-xl font-semibold ${isDark ? 'text-geo-text-main' : 'text-slate-900'}`}>语义词包扩展</h1>
          <span className={`text-sm ${isDark ? 'text-geo-text-sec' : 'text-slate-500'}`}>将语义关键词与自定义内容组合，造句生成普通词包</span>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-8">
        <div className="max-w-[1600px] mx-auto space-y-6">
          {/* 语义词包语义关键词 */}
          <div className={`rounded-2xl border p-4 ${isDark ? 'bg-geo-card border-geo-border' : 'bg-slate-50 border-slate-200'}`}>
            <label className={`block text-xs font-semibold mb-3 ${isDark ? 'text-geo-text-sec' : 'text-slate-500'}`}>
              语义关键词（点击插入到核心词）
            </label>
            {isLoadingKeywords ? (
              ...
              <div className="flex flex-wrap gap-2">
                {semanticKeywords.map((kw, idx) => (
                  <button
                    key={`${kw.text}-${idx}`}
                    type="button"
                    onClick={() => insertKeyword(kw.text)}
                    className={`px-3 py-1.5 rounded-lg text-sm font-semibold transition-all hover-scale
                      ${isDark ? 'bg-geo-blue/20 text-geo-blue hover:bg-geo-blue/30 border border-geo-blue/30' : 'bg-blue-50 text-blue-700 hover:bg-blue-100 border border-blue-200'}
                    `}
                  >
                    {kw.text}
                  </button>
              ...
              <p className={`text-sm ${isDark ? 'text-geo-text-sec' : 'text-slate-500'}`}>该语义词包暂无语义关键词</p>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
            <div className="flex flex-col min-w-0">
              ...
              <textarea value={region} onChange={e => setRegion(e.target.value)} className={`${inputClasses} min-h-[50vh]`} placeholder={'北京\n上海\n广州'} />
            </div>
            <div className="flex flex-col min-w-0">
              <label className={`block text-xs font-semibold mb-2 shrink-0 ${isDark ? 'text-geo-text-sec' : 'text-slate-500'}`}>前缀词</label>
              <textarea value={prefix} onChange={e => setPrefix(e.target.value)} className={`${inputClasses} min-h-[50vh]`} placeholder="口碑好的、靠谱的..." />
            </div>
            <div className="flex flex-col min-w-0">
              <label className={`block text-xs font-semibold mb-2 shrink-0 ${isDark ? 'text-geo-text-sec' : 'text-slate-500'}`}>核心词（必填，支持语义关键词+自定义组合）</label>
              <textarea value={coreWords} onChange={e => setCoreWords(e.target.value)} className={`${inputClasses} min-h-[50vh]`} placeholder={'点击上方语义关键词插入\n或输入自定义内容\n每行一个复合词'} />
            </div>
            <div className="flex flex-col min-w-0">
              <label className={`block text-xs font-semibold mb-2 shrink-0 ${isDark ? 'text-geo-text-sec' : 'text-slate-500'}`}>行业词</label>
              <textarea value={industry} onChange={e => setIndustry(e.target.value)} className={`${inputClasses} min-h-[50vh]`} placeholder="厂家、供应商..." />
            </div>
            <div className="flex flex-col min-w-0">
              <label className={`block text-xs font-semibold mb-2 shrink-0 ${isDark ? 'text-geo-text-sec' : 'text-slate-500'}`}>后缀词</label>
              <textarea value={suffix} onChange={e => setSuffix(e.target.value)} className={`${inputClasses} min-h-[50vh]`} placeholder="找哪家、怎么联系..." />
            </div>
          </div>
          ...
          <div className={`rounded-xl border p-4 ${isDark ? 'border-geo-border bg-geo-card/50' : 'border-amber-200 bg-amber-50/80'}`}>
            <p className={`text-sm ${isDark ? 'text-geo-text-sec' : 'text-amber-900'}`}>
              <span className="font-semibold">如何保存为普通词包：</span>
              先点击下方「预览拓词」生成词条列表；列表出现后，右侧会出现「保存为普通词包」按钮（供「内容生成」选用）。
            </p>
          </div>
          ...
          <div className={`border-t pt-6 ${isDark ? 'border-geo-border' : 'border-slate-200'}`}>
              <div className="flex justify-between items-center mb-3 flex-wrap gap-3">
                <p className={`text-sm font-bold ${isDark ? 'text-geo-text-sec' : 'text-slate-500'}`}>
                  {result.length > 0 ? `已生成 ${result.length} 条` : '尚未生成词条，请先「预览拓词」'}
                </p>
                <div className="flex gap-2 flex-wrap">
                  ...
                  >
                    {saving ? <span className="animate-pulse">保存中...</span> : <><Save className="w-4 h-4" /> 保存为普通词包</>}
                  </button>
                  <button
                    type="button"
                    onClick={handleCopyAll}
                    disabled={result.length === 0}
                    className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold disabled:opacity-40 ${isDark ? 'bg-geo-card hover:bg-geo-border text-geo-text-sec' : 'bg-slate-100 hover:bg-slate-200 text-slate-600'}`}
                  >
              ...
              <div className={`max-h-80 overflow-y-auto rounded-xl border p-4 space-y-1 min-h-[4rem] ${isDark ? 'bg-geo-card/50 border-geo-border' : 'bg-slate-50 border-slate-200'}`}>
                {result.length === 0 ? (
                  <p className={`text-sm ${isDark ? 'text-geo-text-sec' : 'text-slate-400'}`}>预览结果将显示在此处</p>
                ) : (
                  result.map((q, idx) => (
...
export default SemanticPackExpandPage;
