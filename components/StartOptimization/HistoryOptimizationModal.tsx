...
interface Props {
  ...
  onCancel: () => void;
  /** 进入已有工作流工作台 */
  onEnterWorkflow: (workflowId: string) => void;
}

/**
 * 按品牌拉取历史 GEO 工作流，按「更新时间」倒序，每页 5 条
 */
const HistoryOptimizationModal: React.FC<Props> = ({ brand, onCancel, onEnterWorkflow }) => {
    ...
    >
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 shrink-0">
          <div>
            <div className="text-sm font-semibold text-gray-900">历史优化</div>
            <div className="mt-0.5 text-[11px] text-gray-400">品牌：{brand.name} · 按最近更新时间倒序，每页 {PAGE_SIZE} 条</div>
          </div>
          <button
            type="button"
            onClick={onCancel}
        ...
        <div className="flex-1 min-h-0 overflow-y-auto px-5 py-4">
          {loading && items.length === 0 ? (
            ...
            <div className="text-center text-sm text-gray-400 py-12">该品牌暂无历史优化工作流</div>
          ) : (
            <ul className="space-y-2">
              {items.map((wf) => (
                <li key={wf.workflowId}>
                  <button
                    type="button"
                    onClick={() => onEnterWorkflow(wf.workflowId)}
                    className="w-full text-left rounded-xl border border-gray-100 bg-gray-50/80 hover:bg-[#FFF9F6] hover:border-[#E8553F]/30 px-4 py-3 transition-colors"
                  >
                    <div className="flex flex-wrap items-baseline justify-between gap-2">
                      <div className="min-w-0">
                        ...
                        {wf.productName ? (
                          <span className="text-xs text-gray-600 mt-0.5 block truncate">产品：{wf.productName}</span>
                        ) : null}
                      </div>
...
export default HistoryOptimizationModal;
