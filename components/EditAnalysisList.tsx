/**
 * 编辑诊断报告 - 列表页
 * Admin 专用：查看并编辑系统全部诊断报告，支持指派给用户
 */

import React, { useState, useEffect, useCallback } from 'react';
import { FileBarChart, Loader2, Edit3, UserPlus, X } from 'lucide-react';
...
interface EditAnalysisListProps {
  theme: Theme;
}
...
const EditAnalysisList: React.FC<EditAnalysisListProps> = ({ theme }) => {
  const isDark = theme === 'dark';
  const [reports, setReports] = useState<DiagnosisReportData[]>([]);
  const [total, setTotal] = useState(0);
  const [loading, setLoading] = useState(true);
  const [page, setPage] = useState(1);
  const [error, setError] = useState<string | null>(null);
  const [editingReportId, setEditingReportId] = useState<number | null>(null);
  const [assignModal, setAssignModal] = useState<{ reportId: number; brandName: string } | null>(null);
  ...
  const loadReports = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await diagnosisReportAPI.listAll((page - 1) * PAGE_SIZE, PAGE_SIZE);
  ...
  const openAssignModal = async (reportId: number, brandName: string) => {
    setAssignModal({ reportId, brandName });
    setAssignError(null);
    try {
  ...
  const handleAssign = async (userId: number) => {
    ...
    setAssignError(null);
    try {
      await diagnosisReportAPI.assign(assignModal.reportId, userId);
      closeAssignModal();
      loadReports();
    } catch (e) {
    ...
    <div className={`p-6 ${isDark ? 'bg-geo-bg' : 'bg-slate-50'}`}>
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center gap-3 mb-6">
          <FileBarChart className={`w-6 h-6 ${isDark ? 'text-blue-400' : 'text-blue-600'}`} />
          <h2 className={`text-xl font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>
            编辑诊断报告
          </h2>
        </div>
        <p className={`text-sm mb-6 ${isDark ? 'text-zinc-400' : 'text-slate-500'}`}>
          查看并编辑系统内全部诊断报告，可将现状分析与诊断报告指派给指定用户。
        </p>
        ...
        {loading ? (
          ...
          <div className={`rounded-2xl border p-12 text-center ${isDark ? 'bg-zinc-900/50 border-white/10' : 'bg-white border-slate-200'}`}>
            <FileBarChart className={`w-12 h-12 mx-auto mb-4 ${isDark ? 'text-zinc-600' : 'text-slate-300'}`} />
            <p className={`font-semibold ${isDark ? 'text-zinc-400' : 'text-slate-500'}`}>暂无诊断报告</p>
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className={isDark ? 'bg-zinc-800/50 text-zinc-400' : 'bg-slate-50 text-slate-500'}>
                  <th className="text-left px-4 py-3 font-semibold">批次/任务</th>
                  <th className="text-left px-4 py-3 font-semibold">品牌</th>
                  <th className="text-left px-4 py-3 font-semibold">可见度</th>
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex justify-end gap-2">
                        <button
                          onClick={() => openAssignModal(r.id, r.brandName || r.brandId || '')}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={closeAssignModal}>
            ...
            >
              <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200/50">
                <h3 className={`text-lg font-semibold ${isDark ? 'text-white' : 'text-slate-900'}`}>指派报告</h3>
              ...
              <div className="px-6 py-4">
                <p className={`text-sm mb-4 ${isDark ? 'text-zinc-400' : 'text-slate-500'}`}>
                  将「{assignModal.brandName}」的现状分析与诊断报告指派给：
                </p>
                {assignError && (
...
export default EditAnalysisList;
